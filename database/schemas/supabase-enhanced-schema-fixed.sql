-- ESQUEMA SUPABASE REDESIGNADO - VERSÃO CORRIGIDA
...
-- Modelos de aparelhos (criar primeiro)
...
-- Locais de armazenamento padrão
INSERT INTO storage_locations (id, household_id, name, type, description) VALUES
('default-fridge', (SELECT id FROM households LIMIT 1), 'Geladeira Principal', 'fridge', 'Geladeira principal da cozinha'),
('default-freezer', (SELECT id FROM households LIMIT 1), 'Freezer Principal', 'freezer', 'Freezer principal'),
('default-pantry', (SELECT id FROM households LIMIT 1), 'Despensa', 'pantry', 'Armário de despensa')
...
DROP POLICY IF EXISTS "Users can view household" ON households;
...
DROP POLICY IF EXISTS "Users can manage household" ON households;
...
DROP POLICY IF EXISTS "Users can view own shopping_lists" ON shopping_lists;
CREATE POLICY "Users can view own shopping_lists" ON shopping_lists FOR SELECT USING (
  household_id IN (SELECT household_id FROM profiles WHERE id = auth.uid())
);

DROP POLICY IF EXISTS "Users can manage own shopping_lists" ON shopping_lists;
CREATE POLICY "Users can manage own shopping_lists" ON shopping_lists FOR ALL USING (
  household_id IN (SELECT household_id FROM profiles WHERE id = auth.uid())
);
